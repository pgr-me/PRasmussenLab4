...
def heapify(li: list, parent_ix, n_items_to_sort: Union[int, None]):
    ...
    max_val_ix = parent_ix  # Initialize max_val_ix as parent
    left_ix = 2 * parent_ix + 1  # left_ix = 2*i + 1
    right_ix = 2 * parent_ix + 2  # right_ix = 2*i + 2
...
def heap_sort(li, n_items_to_sort=None):
    ...
    Original function modified to make code easier to read.
    """
    # Set n_items_to_sort to length of li if not specified
    if n_items_to_sort is None:
        n_items_to_sort = len(li)

    # Make sure n_items_to_sort is not greater than length of li
    elif n_items_to_sort > len(li):
        raise HeapError(
            "n_items_to_sort {n_items_to_sort} should not be greater than length of li.")
